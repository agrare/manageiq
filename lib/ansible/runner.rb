  ...
  class Runner
    class << self
      def available?
        ...
        @available = system("which ansible-runner &>/dev/null")
      end
      ...
      def run_via_cli(hosts, credentials, env_vars, extra_vars, tags: nil, ansible_runner_method: "run", verbosity: 0, become_enabled: false, **playbook_or_role_args)
        ...
        begin
          ...
          res = response(base_dir, ansible_runner_method, result, debug).tap { |res| puts res.command_line; puts env_vars_hash }
        ensure
